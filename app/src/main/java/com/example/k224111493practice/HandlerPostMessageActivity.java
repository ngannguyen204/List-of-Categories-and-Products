...
public class HandlerPostMessageActivity extends AppCompatActivity {
    ...
    Runnable foregroundThread = new Runnable() {
        ...
        public void run() {
            ...
            //UPDATE UI …
            if(ti_in_background!=null)//có dữ liệu từ tiểu trình gửi về
            {
        ...
        Thread backgroundThread = new Thread(new Runnable() {
            ...
            public void run() {
                for(int i=1; i<=numb; i++){
                    ...
                    ti_in_background=new TelephonyInfor();
                    ti_in_background.setDisplayName("Name"+i);
                    String []providers={"098","097","096","090","091","093","094"};
                    String phone=providers[random.nextInt(providers.length)];
                    for (int p=1;p<=7;p++)
                        phone+=random.nextInt(10);
                    ti_in_background.setPhoneNumber(phone);
